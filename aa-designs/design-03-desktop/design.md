...
a library, the file system). It hides *how* the outside work is done. The action
calls the gateway and gets back a clean result.

1. One gateway covers one outside concern (one API, one store, one service group).
2. Only make a gateway when there is real outside work to do. Do not make empty
   gateways.
3. The gateway shows the action only the few methods it needs. It hides all the
   messy details inside.
4. The gateway takes ready-to-use input from the action and passes it to the core
   service. It does not re-shape the data itself.
5. The gateway returns clean results (plain values or data models), never raw
   replies from the outside service.
6. The gateway holds **no business logic**. It only passes the call along and
   tidies up the result.
7. The gateway never touches the screen, the controllers, the handlers, or the
   state.
8. Keep the gateway's methods the **same** whether the work is done locally or far
   away (local library now, remote API later). Then the action never has to change.
9. All gateways are gathered into one bundle, built once at startup and handed to
   the app.
...
1. Core has two main parts: **services** (do the work) and **models** (hold the
   data shapes).
2. A **model** is a simple, read-only data holder. It just stores values. It has
   no logic and does no saving or formatting.
3. Give models easy "factory" shortcuts to create them (like `Message.user(text)`),
   and keep all the fixed text constants (like role names) in one small place.
4. A **service** does one clear job (call the AI, read a PDF, build an index).
5. All the outside-library details (the API client, the PDF reader) live **inside**
   Core services. Nothing outside Core imports those libraries.
6. A service takes ready input and returns a plain value or a data model.
7. Core never touches the screen, the state, the actions, the gateways, or the
   startup code.
8. Core does not run workflows (the step-by-step "first this, then that"). That is
   the action's job. Core only holds the rules and the data.
9. Core must work the same way whether the app runs locally or remotely.
...
Config (startup only)
  ...
  → builds Gateways
  → hands Gateways to the app

Event → Handler → Action → Gateway → Core
                    │
                    └→ State Controller (change data)
Core → Gateway → Action → (result) → Handler → screen
```

- **Config** sets things up once, at the start.
- **Core** does the real work and holds the data shapes.
- **Gateways** are the doors to the outside, so Core can be swapped or moved.
- **Actions** are the workers that run one job, change the data, and call a gateway.
...
settings, builds the outside-world doors (gateways) for this mode, and then starts
the Main Controller.
...
3. It builds the **core services** from the config (the AI client, the file reader,
   and so on).
4. It builds the **gateways** for this mode (local now, remote later) and puts them
   in one bundle. **This is the only place that decides local vs remote.**
5. It creates the Main Controller, hands it the gateways, and tells it to start.
6. The launcher does **no business logic** and **no wiring** of signals. It only
   sets up and hands off.
7. To run a different mode (for example a remote version), you write a **different
...
│
├── core/                       # the real work + data shapes (knows nothing above it)
│   ├── services/               # do the work; hide outside libraries here
...
2. **Lower layers never import higher layers.** UI does not import handlers; core
   does not import actions; and so on. Imports only point downward.
3. `conf/` is read only by the launcher. No other folder imports it.
4. `core/` sits at the bottom and depends on nothing else in the app.
5. `desktop/` is the same no matter where the app runs. The mode is chosen only in
   the launcher folders (`desktop_local/`, `desktop_remote/`).
6. `ui/` is also the same in every mode. It hands out a bundle of controllers and
   nothing more.
7. Each layer hands the next one a **bundle** (a gateway bundle, an action bundle,
   a UI bundle) so the wiring stays simple.
...
Then, for each user action:
   Event → Handler → Action → Gateway → Core
                       └→ State Controller (change data)
   Core → Gateway → Action → result → Handler → update screen
```

- The **launcher** starts everything and picks the mode.
- The **Main Controller** wires everything together, once.
- After that, every user action flows down the layers and back up the same path.
