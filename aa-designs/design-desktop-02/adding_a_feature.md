...
2. Core            → model(s) + service                (core_rules.md)
...
10. Launcher        → load config → build service + gateway (main_controller_rules.md, Part A)
```

Config comes first because the Core service it feeds (step 2) is built from config
primitives in the launcher (step 10). Lower layers (1–5) are pure logic with no UI
dependency, so they can be built and unit-tested before the UI (6–10) connects them
to the user.
...
### 2. Core — model(s) + service
*(core_rules.md)*

- Add a **frozen** `@dataclass` model if there is new domain data. Give it factory
  constructors; no `to_dict()`, no logic.
- Add a **service** for the new capability under `core/services/<area>/`. It builds
  its own provider client from primitives, owns all third-party library specifics,
  takes pre-shaped input, returns plain values or domain models.
- Skip this step if the feature reuses existing models/services.
...
- One workflow, one `execute(...)`, constructed `(state_controller, gateways)`.
...
### 10. Launcher — load config + build service + gateway
*(main_controller_rules.md, Part A)*

- `load_config()` already returns the new settings (from step 1) on the bundle.
- Build the new Core service from config primitives.
- Build/extend the gateway and add it to `GatewayBundle`.
- This is the **only** place config is read and Core services / gateways are
  constructed.
...
2. Core      → Document, Chunk, ProjectIndex models;
               extractor, chunker, embedding, vector_store services
3. Gateway   → new IndexGateway (build_index, retrieve) + bundle field
...
10. Launcher → load RetrieverConfig; build the four RAG services + IndexGateway
```
