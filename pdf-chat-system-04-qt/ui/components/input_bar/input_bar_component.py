# ui/components/input_bar/input_bar_component.py

from PyQt6.QtWidgets import QWidget, QHBoxLayout
from PyQt6.QtCore import pyqtSignal

from ui.components.input_bar.widgets.text_input_widget import MessageInputWidget
from ui.components.input_bar.widgets.button_widget import SendButtonWidget


class InputBarComponent(QWidget):
    """
    Composes MessageInputWidget + SendButtonWidget.
    Single responsibility: wire the two children together and expose one signal upward.
    """
    ...
    def _on_send(self):
        """Single handler for both the Enter key and the button click."""
        text = self._message_input.get_text()
        if text:
            self.send_clicked.emit(text)

    # --- Public operations (called by InputBarController) ---
